...
export default function FilingType({ result }: { result: any }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      <SectionTitle icon="📋" label={t("sectionNmpaFilingType")} tooltip={t("tooltipNmpaFilingType")} />
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div className="bg-green-50 rounded-lg p-3 border border-green-200"><p className="text-xs font-bold text-green-800">{t('ordinaryLabel')} {f.ordinary?.includes('Notification') ? '(备案)' : ''}</p><p className="text-[10px] text-green-700 mt-1">{f.ordinary}</p></div>
