...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const url = `https://sinotradecompliance.com/${locale}/services/cosmetics/`;
  return {
    title,
    description,
...
export default async function CosmeticsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const definitionTerms = [    { nameKey: "nmpaName", definitionKey: "nmpaDefinition" },
  ...
  return (
    <main>
      ...
      <HowToJsonLd locale={locale} namespace="ServiceCosmetics" url={url} />
      <Hero
        title={t('heroTitle')}
        subtitle={t('heroSubtitle')}
        ctas={[{ text: t('cta'), href: WHATSAPP_URL, variant: 'primary' }]}
      />
      <QuickAnswer locale={locale} namespace="ServiceCosmetics" />
      <CoverSection t={t} />
      <ProcessSteps t={homeT} />
      <WhyUsCards t={commonT} />
      <ServiceFAQ namespace="ServiceCosmetics" />
      <RelatedResources locale={locale} category={getBlogCategoryForService('cosmetics', locale)} />
      <PortalCTASection t={commonT} href={`/${locale}/c/check/nmpa`} />
      <ContactForm />
      <CTASection t={ctaT} />
          <script id="jsonld-cosmetics" type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} />
          <script id="jsonld-local-cosmetics" type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(localBusinessJsonLd) }} />
    </main>
  );
}
