...
export default function TestingProcess({ result }: { result: any }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      <SectionTitle icon="🔬" label={t("sectionTestingProcess")} />
      <div className="space-y-3">
        {result.testingProcess.map((item: any, i: number) => (
          <div key={i} className="flex items-start gap-3">
            ...
            <div className="flex-1 bg-gray-50 rounded-lg p-3">
              <div className="flex justify-between items-start">
                <h3 className="text-sm font-semibold text-gray-900">{item.phase}</h3>
                <span className="text-xs font-bold text-gold bg-gold/10 px-2 py-0.5 rounded">{localizeTimeline(t, item.duration)}</span>
              </div>
