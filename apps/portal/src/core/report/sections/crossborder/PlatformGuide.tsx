...
export default function PlatformGuide({ result }: { result: any }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      <SectionTitle icon="🛒" label={t("sectionPlatformComparison")} tooltip={t("tooltipPlatformComparison")} />
      <div className="overflow-x-auto -mx-6">
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50">
              <th className="text-left px-6 py-2.5 font-semibold text-gray-700">{t("labelPlatform")}</th>
              <th className="text-right px-4 py-2.5 font-semibold text-gray-700">{t("labelFee")}</th>
              <th className="text-left px-4 py-2.5 font-semibold text-gray-700 hidden md:table-cell">{t("labelRequirement")}</th>
              <th className="text-left px-4 py-2.5 font-semibold text-gray-700 hidden md:table-cell">{t("labelTimeline")}</th>
            </tr>
          </thead>
          <tbody>
      ...
      {result.platformGuide.length >= 2 && (
        <div className="mt-4 bg-gray-50 rounded-lg p-3 border border-gray-200">
          <p className="text-xs font-semibold text-gray-700 mb-2">📊 {t("cbecFeeTimelineGlance")}</p>
          {result.platformGuide.map((p: any, i: number) => {
